...
public class ServiceProvidersController(AppDbContext context, IServiceProviderService providerService) : ControllerBase
{
    ...
    public async Task<IActionResult> GetServiceProviderById(Guid id)
    {
        var p = await context.Providers
            .Include(x => x.User)
            .Include(x => x.ProviderCategories)
        ...
        return Ok(new
        {
            ...
            specialties = p.ProviderCategories
                .Where(pc => pc.Status == Enums.CategoryStatus.Active)
                .Select(pc => pc.Category.Name)
                .ToList(),
            categories = p.ProviderCategories
                .Where(pc => pc.Status == Enums.CategoryStatus.Active)
                .Select(pc => new { id = pc.CategoryId, name = pc.Category.Name })
                .ToList()
        });
    }
